...
<section ng-controller="cart-controller">
	...
	<div class="container">
		...
		<span class="qtdprod" title="5 produto no carrinho"><small>
		...
		<div class="row text-center title-default-roxo " style="margin:40px auto;">
			<div>
			<i class="bi-cart bi-cart-prod"></i>
			<span class="qntd-carrinho"> {{carrinho.qtdprod}} </span>
			</div>
		...
		<table id="cart-products" class="table-responsive table table-hover">
			<thead>
				<tr>
			...
			<tbody>
				<div class="row">
					<tr ng-repeat="produto in produtos">
						<td class="text-center"><img img class="img-fluid" src="img/produtos/{{produto.foto_principal}}"></td>
						<td>{{produto.nome_prod_long}}</td>
						<td class="col-xs-2">
							<div class="input-group">
								<span class="input-group-btn">
									<button class="btn text-roxo" ng-click="removeQtd(produto)" type="button"><i class="fa fa-chevron-down" onClick="window.location.reload()"></i></button>
								</span>
								<input type="text" class="form-control" ng-model="produto.qtd_car">
								<span class="input-group-btn">
									<button class="btn text-roxo" ng-click="addQtd(produto)" type="button"><i class="fa fa-chevron-up" onClick="window.location.reload()"></i></button>
								</span>
							</div>
						</td>
			...
			<div class="col-sm-4">
				<div class="box-cart-totais">
					<table class="table">
						...
						<button type="button" class="btn btn-roxo pull-right"><a href="header.php"></a> Continue Comprando</button>
					</table>
				</div>
			</div>
		</div>
...
<script>
	angular.module("cart", []).controller("cart-controller", function($scope, $http) {

		/* Início Carregando Carrinho */
		var carregarCarrinho = function() {
			...
			$http({
				...
				$scope.carrinho = {
					...
					qtdprod: response.data.qtd_prod,
					total: response.data.total_car,
					prazo: response.data.prazo_car + ' dias úteis' /* Add descobrir */
				};

				$scope.produtos = response.data.produtos;

				console.log(response.data.qtd_prod)
